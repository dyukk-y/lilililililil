...
SETTINGS_SCHEMA: Dict[str, Tuple[type, str]] = {
    ...
    "FIRST_COMMENT_TEXT": (str, "Текст первого комментария под постом"),
    "FIRST_COMMENT_BTN_OFFER_TEXT": (str, "Текст кнопки «Предложить пост»"),
    "FIRST_COMMENT_BTN_OFFER_URL": (str, "Ссылка кнопки «Предложить пост»"),
    "FIRST_COMMENT_BTN_STARS_TEXT": (str, "Текст кнопки «Купить звёзды»"),
    "FIRST_COMMENT_BTN_STARS_URL": (str, "Ссылка кнопки «Купить звёзды»"),
    "FIRST_COMMENT_BTN_VPN_TEXT": (str, "Текст кнопки «Купить VPN»"),
    "FIRST_COMMENT_BTN_VPN_URL": (str, "Ссылка кнопки «Купить VPN»"),
}
...
def display_value_short(key: str, limit: int = 28) -> str:
    """Как display_value(), но обрезает длинные строки — используется в
    кнопках списка настроек, чтобы длинный текст (например,
    FIRST_COMMENT_TEXT) не растягивал кнопку на весь экран."""
