...
from app.keyboards import settings_list_keyboard, settings_edit_cancel_keyboard, admin_menu
...
async def settings_list(cb: CallbackQuery):
    ...
    keys_with_labels = [
        (key, f"{rs.get_label(key)}: {rs.display_value_short(key)}")
...
async def settings_edit_process(msg: Message, state: FSMContext):
    ...
    keys_with_labels = [
        (k, f"{rs.get_label(k)}: {rs.display_value_short(k)}")
