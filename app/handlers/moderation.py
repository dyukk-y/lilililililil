...
async def who_published(cb: CallbackQuery):
    ...
    if moderator_id:
        text += f"🔗 <b>Ссылка:</b> tg://user?id={moderator_id}"
...
async def confirm_pub(cb: CallbackQuery):
    ...
    # Раньше здесь была ОТДЕЛЬНАЯ копия логики публикации прямо в этом файле,
    # которая публиковала пост в канал, но НЕ сохраняла channel_message_id —
    # из-за этого бот не мог найти уже опубликованный пост по ссылке/пересылке
    # (заявки на удаление и поиск автора не работали). Теперь путь только один.
...
async def reject(cb: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=get_setting("MODERATORS_CHAT_ID"),
        message_thread_id=get_setting("MODERATORS_TOPIC_ID"),
