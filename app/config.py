...
# Нормализуем список администраторов: допускаются пробелы, переносы строк
# и случайный BOM в начале .env. Это особенно важно после ручного редактирования
# файла на Windows/через панель хостинга.
ADMINS = []
for _uid in _ADMINS_STR.replace("\ufeff", "").replace("\n", ",").split(","):
    _uid = _uid.strip()
    if not _uid:
        continue
    try:
        ...
        raise ValueError(f"Некорректный Telegram user_id в ADMINS: {_uid!r}") from exc
if not ADMINS:
    raise ValueError(
...
DB_NAME = os.getenv("DB_NAME", "/app/data/smotrbot.db")
