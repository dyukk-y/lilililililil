...
async def get_text_after_photo(msg: Message, state: FSMContext):
    ...
    if not await _passes_auto_checks(msg, state):
        return
    ...
    post_id = await create_post(msg.from_user.id, msg.text, data["photo"])
...
async def _passes_auto_checks(msg: Message, state: FSMContext) -> bool:
    """Стоп-слова (маты/оскорбления/спам) + валидация текста (в т.ч. минимум
    3 слова) + проверка на осмысленность + проверка на повторяющийся
    (нечётко похожий) текст. Возвращает False, если пост нужно отклонить
    ещё до создания записи в БД (пользователю уже отправлен ответ)."""
    ...
    if msg.from_user.id not in ADMINS:
        is_gibberish, gibberish_reason = detect_gibberish(msg.text)
        if is_gibberish:
            await msg.answer(
                "❌ <b>Публикация отклонена</b>\n\n"
    ...
    duplicate_count = await count_similar_posts(msg.text)
