...
async def main() -> None:
    ...
    # Бесплатный локальный ИИ: модель загружается заранее, чтобы первый
    # пользователь не ждал скачивания/инициализации модели. Никаких API-
    # ключей и платных внешних сервисов не используется. Если модель
    # недоступна, бот продолжает работу с безопасным эвристическим fallback.
    if LOCAL_AI_PRELOAD:
        ai_ready = await preload_local_ai()
        logger.info("Локальный ИИ: %s", "готов" if ai_ready else "недоступен, используется fallback")
    else:
        logger.info("Локальный ИИ: ленивая загрузка (экономия RAM)")
