...
async def _admin_panel_text() -> str:
    ...
    return (
        f"🛠 <b>Центр управления</b>\n\n"
        f"<blockquote>Панель администратора: публикации, модерация, пользователи и настройки.</blockquote>\n\n"
        f"📊 <b>Состояние:</b>\n"
        ...
        f"⏳ В очереди сейчас: <b>{queue_count}</b>\n\n"
        f"<blockquote>🤖 <b>ИИ за 30 дней</b> · автопроверка <b>{ai_coverage:.0f}%</b> из анализированных постов.</blockquote>\n\n"
        f"<blockquote>🤖 <i>Посты с ключевыми фразами (см. «Фразы для автопубликации») "
        f"публикуются автоматически по расписанию; всё остальное — фото и "
        f"текст без таких фраз — ждёт решения модератора (до 24 часов).</i></blockquote>\n\n"
